...
foldercount = len(os.listdir(sourcedir))
testcount = args.test * foldercount
datacount = 0
processeddirectories = 0
dir_list = os.listdir(sourcedir)
shuffle(dir_list)
for directory in dir_list:
    if (processeddirectories < testcount):
        targetset = 'test'
    else:
        targetset = 'trainval'
    for subroot, subdirs, subfiles in os.walk(os.path.join(sourcedir,directory)):
        ...
        for name in subfiles:
            ...
            if (ext.lower() == '.jpg' and os.path.exists(os.path.join(subroot, name + '.xml')) and infoldercount < args.maxframes):
                ...
                assert os.path.exists(os.path.join('builds', timestamp, 'test', 'image', name + ext)) == False
                assert os.path.exists(os.path.join('builds', timestamp, 'test', 'label', name + '.xml')) == False
                copyfile(os.path.join(subroot, name + ext), os.path.join('builds', timestamp, targetset, 'image', name.replace(" ", "_") + ext))
                copyfile(os.path.join(subroot, name + '.xml'), os.path.join('builds', timestamp, targetset, 'label', name.replace(" ", "_") + '.xml'))
    processeddirectories += 1
print 'moved ' + str(datacount) + ' images (with annotations) to trainval and test folders'


# resize all images and labels
print 'resizing all images (and annotations)...'
for imageset in ['trainval', 'test']:
    files = os.listdir(os.path.join('builds', timestamp, imageset, 'label'))
    for name in files:
        tree = ET.parse(os.path.join('builds', timestamp, imageset, 'label', name))
        root = tree.getroot()
        ...
        tree.write(os.path.join('builds', timestamp, imageset, 'label', name))
        name, ext = os.path.splitext(name)
        ...
        image.save(os.path.join('builds', timestamp, imageset, 'image', name + '.jpg'))
